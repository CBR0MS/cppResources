...
void addNodesToGraph(int A, vector<pair<int, int>>& p_pairs, map<char, set<int>>& p_map, const char& loc) {
	...
	if (res.size() == 0) { // the branch is ended because it has no links
		...
		for (int i = 0; i < res.size(); i++) { // iterating through the instances of that node
			...
			p_pairs.erase(p_pairs.begin() + res.at(i)); // once we've used a pair, we want to delete it.

			// we want to pass the value in the pair that isn't already in
			// the sub-container, so we'll use the find() function to seek it. 

			auto it = find(p_map[loc].begin(), p_map[loc].end(), next);

			if (it != p_map[loc].end()) {

				it = find(p_map[loc].begin(), p_map[loc].end(), next2);

				if (it == p_map[loc].end()) {

					addNodesToGraph(next2, p_pairs, p_map, loc); // now we recursively pass the new node
				}
			}
			else {
				addNodesToGraph(next, p_pairs, p_map, loc);
			}
			res = findInPairs(A, p_pairs);
		}
	}
}
